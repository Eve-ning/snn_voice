...
def latency_replica(x: torch.Tensor, n_steps: int) -> torch.Tensor:
    x = x[:, 0].abs()
...
def rate_replica(x: torch.Tensor, n_steps: int) -> torch.Tensor:
    x = x[:, 0].abs()
    x = (x - x.min()) / (x.max() - x.min())
    return rate(x, num_steps=n_steps).unsqueeze(2)


def repeat_replica(x: torch.Tensor, n_steps: int) -> torch.Tensor:
